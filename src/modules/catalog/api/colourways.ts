...
// Wire contract for Tiendanube's PRODUCT CUSTOM FIELDS â€” a resource of its
...
// - `color_name` is the established Product Custom Field key. The adapter accepts only that key to avoid a dual-read migration.

/**
 * One product's own colourway. `object[]` is the field's declared type, so a
 * value always arrives as an array â€” carrying exactly one entry, because a
...
/**
 * `entity_id` is a STRING here while `product.id` is a NUMBER â€” the join
 ...
 * owners for a two-product catalogue â€” the other two ids answer
...
export function toColourwayIndex(field: ColourwayField): Map<number, Colourway> {
  ...
  for (const owner of field.owners) {
    ...
    // field as a list of siblings â€” the shape this model deliberately rejects
    // â€” and picking the first would quietly choose a colour for them.
